...
def generate_bank_client_data(num_companies=100, days=1460):  # 4 years of data
    """
...
def plot_heatmap_persona_risk_timing(timing_df, title="Heatmap: Persona-Risk Relationship to Downgrade Timing"):
    ...
    # Check if we have any non-NaN data to plot
    ...
    # Make sure we have at least some valid data points
    if (~mask).sum().sum() == 0:
        print("No valid data points for heatmap after masking NaN values.")
        return None
    ...
    try:
        ...
        print("This usually happens when there is insufficient data or when all values are NaN.")
        return None
...
def plot_lead_time_by_severity(severity_stats, title="Lead Time by Downgrade Severity"):
    ...
    # Add trend line only if we have enough unique data points for regression
    unique_severities = severity_stats['downgrade_severity'].nunique()
    
    if len(severity_stats) > 1 and unique_severities > 1:
        try:
            # Filter out any NaN values before regression
            valid_data = severity_stats.dropna(subset=['downgrade_severity', 'avg_days_before'])
            
            if len(valid_data) > 1:
                # Make sure the data is valid for regression
                x = valid_data['downgrade_severity'].values
                y = valid_data['avg_days_before'].values
                ...
                if len(x) == len(y) and len(x) > 1:
                    ...
                    ax1.plot(x_range, y_pred, 'r--', alpha=0.7)
            ...
            # If regression fails, just log it and continue without the trend line
...
def plot_confidence_impact(timing_df, title="Impact of Persona Confidence on Downgrade Prediction"):
    ...
    # Create a copy and filter out rows with None/NaN in before_confidence
    timing_df = timing_df.copy()
    timing_df = timing_df.dropna(subset=['before_confidence'])
    
    # Make sure we still have data after filtering
    if timing_df.empty or timing_df['before_confidence'].isna().all():
        print("No valid confidence data available after filtering null values.")
        return None
    
    # Now create confidence bins on valid data only
    # timing_df = timing_df.copy()
...
def integrated_risk_backtest_workflow(bank_data_path=None, credit_score_path=None, generate_synthetic=True, num_companies=100):
    """
